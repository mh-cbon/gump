...
- gump(improve): added support for a new .version.sh file format.
- gump(improve): improved test to run anywhere.
- gump(improve): code lint, test, doc.
- gump(improve): CI now runs on all commits.
- config(break): code refactoring to use plugin oriented architecture
- stringexec(fix): the tempfile is correctly deleted after wait()
